...
export default function ContextualIntelligenceTeaser({ lat, lng, projectId }: Props) {
  ...
  return (
    <div className="bg-ci-green-light rounded-[var(--radius-md)] shadow-[var(--shadow-card)] mb-6 overflow-hidden">
      {/* Header bar */}
      <div className="flex items-center justify-between px-6 py-4 border-b border-ci-green/20">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-full bg-ci-green/15 flex items-center justify-center shrink-0">
            <Brain size={15} className="text-ci-green-dark" />
          </div>
          <div>
            <h3
              className="text-[15px] font-bold text-ci-green-dark leading-tight"
              style={{ fontFamily: 'var(--font-display)' }}
            >
              Contextual Intelligence
            </h3>
            <p className="text-[11px] text-ci-green-dark/60 mt-0.5">
        ...
        <Link
          ...
          className="flex items-center gap-1.5 text-[12px] font-semibold text-ci-green-dark/70 hover:text-ci-green-dark transition-colors shrink-0"
        >
          Full analysis
          <ArrowRight size={13} />
        </Link>
        ...
        {loading ? (
          <div className="space-y-3">
            <div className="flex gap-3">
              <div className="h-4 w-4 rounded bg-ci-green/20 animate-pulse shrink-0 mt-0.5" />
              <div className="space-y-1.5 flex-1">
                <div className="h-3.5 bg-ci-green/20 animate-pulse rounded w-3/4" />
                <div className="h-3 bg-ci-green/10 animate-pulse rounded w-full" />
                <div className="h-3 bg-ci-green/10 animate-pulse rounded w-2/3" />
              </div>
            </div>
            <div className="flex gap-3">
              <div className="h-4 w-4 rounded bg-ci-green/20 animate-pulse shrink-0 mt-0.5" />
              <div className="space-y-1.5 flex-1">
                <div className="h-3.5 bg-ci-green/20 animate-pulse rounded w-1/2" />
                <div className="h-3 bg-ci-green/10 animate-pulse rounded w-5/6" />
              </div>
            </div>
          </div>
        ) : error ? (
          <p className="text-[13px] text-white/40 italic">
            Climate data unavailable — <Link href={`/projects/${projectId}/context`} className="underline text-ci-green-dark/70">view full analysis</Link> to retry.
          </p>
        ) : (
          <>
              ...
              <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[11px] font-semibold ${
                precipAnomaly < -15
                  ...
                  : precipAnomaly > 15
                    ...
                    : 'bg-ci-green/15 text-ci-green-dark border border-ci-green/20'
              }`}>
                <CloudRain size={11} />
            ...
            <div className="space-y-3">
              {visibleInterpretations.map((interp, i) => (
                <div key={i} className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-[13px] font-semibold text-ci-charcoal leading-snug mb-0.5">{interp.headline}</p>
                    <p className="text-[12px] text-ci-gray-700 leading-relaxed">{interp.body}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            {interpretations.length > 2 && (
              <button
                ...
                className="flex items-center gap-1.5 mt-4 text-[11px] font-semibold text-ci-green-dark/60 hover:text-ci-green-dark transition-colors"
              >
                {expanded ? (
